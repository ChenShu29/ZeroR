...
class BaseCameraDataset(basedataset.BaseDataset):
    def __init__(self, params, visualize, train=True, validate=False):
        super(BaseCameraDataset, self).__init__(params, visualize, train, validate)
        self.pre_processor = build_preprocessor(params['preprocess'], train)
        self.post_processor = build_postprocessor(params['postprocess'], train)

    def get_sample(self, idx):
        if self.temporal_params['temporal']:
            base_data_dict = self.retrieve_base_data_temporal(idx, cur_ego_pose_flag=True)
        else:
            base_data_dict = self.retrieve_base_data_temporal(idx, cur_ego_pose_flag=True)
    ...
    def get_sample_debug(self, scenario_idx, timestamp_index):
        """
        ...
        base_data_dict = self.retrieve_base_data(timestamp_index, True)
        return self.get_data_sample(base_data_dict)
    ...
    def get_data_sample(self, base_data_dict):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            object_bbx_ego, object_bbx_cav, object_ids = self.get_item_single_car(selected_cav_base, ego_lidar_pose)
        ...
        object_bbx_center = np.zeros((100, 7))
        mask = np.zeros(100)
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        object_bbx_center_cav, object_bbx_mask_cav, _ = \
            self.post_processor.generate_object_center([selected_cav_base], selected_cav_base['params']['lidar_pose'])

        return object_bbx_center_ego[object_bbx_mask == 1], object_bbx_center_cav[object_bbx_mask_cav == 1], object_ids

    def visualize_agent_camera_bbx(self, agent_sample, camera='camera0', draw_3d=True, color=(0, 255, 0), thickness=2):
        """
        ...
        objects = box_utils.boxes_to_corners_3d(objects, self.post_processor.params['order'])
        # project objects coordinate from lidar space to camera space
        object_camera = camera_utils.project_3d_to_camera(objects, camera_intrinsic, camera_extrinsic)
        if draw_3d:
            draw_rgb = camera_utils.draw_3d_bbx(camera_rgb, object_camera, color, thickness)
        else:
            draw_rgb = camera_utils.draw_2d_bbx(camera_rgb, objects, color, thickness)
        return draw_rgb

    def visualize_agent_bbx(self, data_sample, agent, draw_3d=True, color=(0, 255, 0), thickness=2):
        """
        ...
        for camera in ['camera0', 'camera1', 'camera2', 'camera3']:
            draw_image = self.visualize_agent_camera_bbx(agent_sample, camera, draw_3d, color, thickness)
            draw_image_list.append(draw_image)

        return draw_image_list

    def visualize_all_agents_bbx(self, data_sample, draw_3d=True, color=(0, 255, 0), thickness=2):
        """
        ...
        for cav_id, cav_content in data_sample.items():
            draw_image_list.append(self.visualize_agent_bbx(data_sample, cav_id, draw_3d, color, thickness))
...
if __name__ == '__main__':
    params = load_yaml('/data0/ychen_data/E2E_CoBEV/opencood/hypes_yaml/_.yaml')

    opencda_dataset = BaseCameraDataset(params, train=True, visualize=True)
    data_example = opencda_dataset.get_sample(4)
    draw_image_list, cav_id_list = opencda_dataset.visualize_all_agents_bbx(data_example)
