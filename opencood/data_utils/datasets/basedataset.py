...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True, validate=False):
        ...
        if self.train and not self.validate:         # (train=Ture, validate=False) -> train_dir
            root_dir = params['root_dir']
        elif self.train and self.validate:           # (train=Ture, validate=True)  -> validate_dir
            root_dir = params['validate_dir']
        elif not self.train and not self.validate:   # (train=False, validate=False) -> test_dir
        ...
        # By default, we load lidar, camera and metadata. But users may define additional inputs/tasks (e.g. Semantic)
        self.add_data_extension = params['add_data_extension'] if 'add_data_extension' in params else []

        # Get input data modality - for later dataset
        self.modality = params['modality']

        self.temporal_params = params['temporal_params']

        # first load all paths of different scenarios
        scenario_folders = sorted([os.path.join(root_dir, x) for x in os.listdir(root_dir) if
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            # Roadside unit data's id is always negative, so here we want to make sure they will be in the end of
            # the list as they shouldn't be ego vehicle.
            # TODO: Judge whether the data should be shuffle
            if int(cav_list[0]) < 0:
                cav_list = cav_list[1:] + [cav_list[0]]

            # if self.train and self.params['base_model']:
            #     cav_list = [random.choice(cav_list[:-1])]
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                # use the frame number as key, the full path as the values (all yaml files under dataset/scenario/cav/)
                ...
                for timestamp in timestamps:
                    ...
                    # Load additional data (e.g. semantic)
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # load the corresponding data into the dictionary - Lidar data, label, transformation matrix etc.
            ...
            if self.modality == 'camera':
                data[cav_id]['camera_params'] = self.reform_camera_param(cav_content, ego_cav_content, timestamp_key)

                # Load camera data to np format
                data[cav_id]['camera_np'] = load_rgb_from_files(cav_content[timestamp_key_delay]['cameras'])
            else:
                # process lidar data to np array
            ...
            for file_extension in self.add_data_extension:
                if '.yaml' in file_extension:
                    ...
                    data[cav_id][file_extension] = np.load(cav_content[timestamp_key][file_extension])
                else:
    ...
    def retrieve_base_data_temporal(self, idx, cur_ego_pose_flag=True):
        """
        Given the index, return the corresponding data.
        ...
        """
        # we loop the accumulated length list to see get the scenario index
        # length_record - the accumulated sample number for each scenario [419, 419+74, 419+74+79, 419+74+79+122, ...]
        scenario_index = 0
        for i, ele in enumerate(self.len_record):
            if idx < ele:
                scenario_index = i
        ...
        # retrieve the corresponding timestamp key (including current, previous, future)
        timestamp_key_current, timestamp_key_previous, timestamp_key_future, start_flag, end_flag \
            = self.return_timestamp_key_temporal(scenario_database, timestamp_index, scenario_index)

        # calculate distance to ego for each cav
        ego_cav_content = self.calc_dist_to_ego(scenario_database, timestamp_key_current)

        # Collect the distances to ego (Only reserve agents that within communication range and most far ones under max_cav)
        distances = [content['distance_to_ego'] for content in scenario_database.values()]
        distances = sorted(distances, reverse=False)
        distances =[x for x in distances if x < opencood.data_utils.datasets.COM_RANGE]

        # Here the max cav have been filtered [Get the most far CAVs less than max_cav]
        distance_threshold = 0 if len(distances) < self.max_cav else distances[-self.max_cav]

        data = OrderedDict()
        # load files for all CAVs
        for cav_id, cav_content in scenario_database.items():
            if cav_content['distance_to_ego'] == 0 or distance_threshold < cav_content['distance_to_ego'] <= opencood.data_utils.datasets.COM_RANGE:
                data[cav_id] = OrderedDict()
                data[cav_id]['ego'] = cav_content['ego']
                ...
                # load the corresponding data into the dictionary - Lidar data, label, transformation matrix etc.
                data[cav_id]['params'] = self.reform_param_temporal(cav_content, ego_cav_content, timestamp_key_current, timestamp_key_delay, cur_ego_pose_flag, timestamp_key_previous[-1])
                data[cav_id]['params']['start_flag'] = start_flag
                data[cav_id]['params']['end_flag'] = end_flag

                if self.modality == 'camera':
                    data[cav_id]['camera_params'] = self.reform_camera_param(cav_content, ego_cav_content, timestamp_key_current)

                    # Load camera data to np format
                    data[cav_id]['camera_np'] = load_rgb_from_files(cav_content[timestamp_key_delay]['cameras'])

                    # Load previous camera data to np format
                    if self.temporal_params['temporal']:
                        data[cav_id]['params_previous'] = OrderedDict()
                        data[cav_id]['camera_params_previous'] = OrderedDict()
                        data[cav_id]['camera_np_previous'] = OrderedDict()
                        for timestamp_key_previous_idx, timestamp_key_previous_str in enumerate(timestamp_key_previous):
                            # TODO: Here we haven't process the motion compensation for previous timestamps now.
                            data[cav_id]['params_previous'][timestamp_key_previous_idx] = self.reform_param_temporal(cav_content, ego_cav_content, timestamp_key_previous_str, timestamp_key_previous_str, cur_ego_pose_flag, timestamp_key_previous_str)
                            data[cav_id]['camera_params_previous'][timestamp_key_previous_idx] = self.reform_camera_param(cav_content, ego_cav_content, timestamp_key_previous_str)
                            data[cav_id]['camera_np_previous'][timestamp_key_previous_idx] = load_rgb_from_files(cav_content[timestamp_key_previous_str]['cameras'])
                ...
                for file_extension in self.add_data_extension:
                    if '.yaml' in file_extension:
                        data[cav_id][file_extension] = load_yaml(cav_content[timestamp_key_current][file_extension])
                    elif '.npy' in file_extension:
                        data[cav_id][file_extension] = np.load(cav_content[timestamp_key_current][file_extension])
                    else:
                        data[cav_id][file_extension] = cv2.imread(cav_content[timestamp_key_current][file_extension])

                # Load additional data for future occupancy label
                if self.temporal_params['temporal']:
                    data[cav_id]['future_dynamic'] = OrderedDict()
                    data[cav_id]['future_lane'] = OrderedDict()
                    data[cav_id]['future_semantic'] = OrderedDict()
                    for timestamp_key_future_idx, timestamp_key_future_str in enumerate(timestamp_key_future):
                        data[cav_id]['future_semantic'][timestamp_key_future_idx] = np.load(cav_content[timestamp_key_future_str]['semantic_2d.npy'])
                        data[cav_id]['future_dynamic'][timestamp_key_future_idx] = cv2.imread(cav_content[timestamp_key_future_str]['bev_visibility_corp.png'])
                        data[cav_id]['future_lane'][timestamp_key_future_idx] = cv2.imread(cav_content[timestamp_key_future_str]['bev_visibility_corp.png'])
    ...
    def return_timestamp_key_temporal(self, scenario_database, timestamp_index, scenario_index):
        """
        Given the timestamp index, return the correct timestamp key, e.g.
        2 --> '000078'.
        ...
        # get all timestamp keys
        timestamp_keys = list(scenario_database.items())[0][1]
        timestamp_keys = list(timestamp_keys.items())

        # retrieve the current index
        timestamp_key_current = timestamp_keys[timestamp_index][0]

        # retrieve the previous index
        timestamp_key_previous = []
        for timestamp in range(timestamp_index - self.temporal_params['previous_step'], timestamp_index):
            if timestamp < 0:
                timestamp_key_previous.append(timestamp_keys[0][0])
            else:
                timestamp_key_previous.append(timestamp_keys[timestamp][0])

        # retrieve the future index
        upper_sample = self.len_record[0] if scenario_index==0 else self.len_record[scenario_index] - self.len_record[scenario_index-1]
        timestamp_key_future = []
        for timestamp in range(timestamp_index+1, timestamp_index+1+self.temporal_params['predict_step']):
            if timestamp < upper_sample:
                timestamp_key_future.append(timestamp_keys[timestamp][0])
            else:
                timestamp_key_future.append(timestamp_keys[upper_sample-1][0])

        return timestamp_key_current, timestamp_key_previous, timestamp_key_future, timestamp_index == 0, timestamp_index == upper_sample - 1

    @staticmethod
    def calc_dist_to_ego(scenario_database, timestamp_key):
        """
    ...
    def reform_camera_param(self, cav_content, ego_content, timestamp):
        ...
        for i in range(4):
            ...
            camera_extrinsic = np.array(cav_params['camera%d' % i]['extrinsic'])
            camera_extrinsic_to_ego_lidar = x1_to_x2(camera_coords, ego_lidar_pose)
            camera_extrinsic_to_ego = x1_to_x2(camera_coords, ego_pose)

            camera_intrinsic = np.array(cav_params['camera%d' % i]['intrinsic'])

            cur_camera_param = {'camera_coords': camera_coords,
                                'camera_extrinsic': camera_extrinsic,
                                'camera_intrinsic': camera_intrinsic,
                                'camera_extrinsic_to_ego_lidar': camera_extrinsic_to_ego_lidar,
                                'camera_extrinsic_to_ego': camera_extrinsic_to_ego}
            camera_params.update({'camera%d' % i: cur_camera_param})

        return camera_params


    def reform_param_temporal(self, cav_content, ego_content, timestamp_cur, timestamp_delay, cur_ego_pose_flag, timestamp_last):
        """
        Reform the data params with current timestamp object groundtruth and delay timestamp LiDAR pose for other CAVs.
        Note: Leverage mainly for LiDA data and label
        ...
        # This is only used for late fusion, as it did the transformation in the postprocess,
        # so we want the gt object transformation use the correct one
        gt_transformation_matrix = x1_to_x2(cur_cav_lidar_pose, cur_ego_lidar_pose)

        last_ego_lidar_pose = load_yaml(cav_content[timestamp_last]['yaml'])['lidar_pose']
        ego_motion_transformation = x1_to_x2(last_ego_lidar_pose, cur_ego_lidar_pose)

        # we always use current timestamp's gt bbx to gain a fair evaluation
        delay_params['vehicles'] = cur_params['vehicles']
        delay_params['transformation_matrix'] = transformation_matrix
        delay_params['gt_transformation_matrix'] = gt_transformation_matrix       # Coordinate transformation
        delay_params['spatial_correction_matrix'] = spatial_correction_matrix     # Mix delay (if not consider delay, all I matrix)
        delay_params['ego_motion_transformation'] = torch.from_numpy(ego_motion_transformation)
        delay_params['TimeStamp'] = timestamp_cur
        delay_params['timestamp_path'] = ego_content[timestamp_cur]['yaml']
    ...
    def find_ego_pose(self, base_data_dict):
        """
