...
class VanillaSegLoss(nn.Module):
    def __init__(self, args):
        ...
        self.loss_func_static = nn.CrossEntropyLoss(weight=torch.Tensor([1., self.s_weights, self.l_weights]).cuda())
        self.loss_func_dynamic = nn.CrossEntropyLoss(weight=torch.Tensor([1., self.d_weights]).cuda())
    ...
    def forward(self, output_dict, gt_dict):
        ...
        # [B, 1(class), H, W]
        static_gt = gt_dict['gt_static'].squeeze(1)
        ...
        else:  # self.target == 'both'
            dynamic_loss = self.loss_func_dynamic(dynamic_pred, dynamic_gt)
            static_loss = self.loss_func_static(static_pred, static_gt)

        total_loss = self.s_coe * static_loss + self.d_coe * dynamic_loss
        self.loss_dict.update({'total_seg_loss': total_loss,
                               'seg_static_loss': static_loss,
                               'seg_dynamic_loss': dynamic_loss})
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        """
        total_loss = self.loss_dict['total_seg_loss']
        static_loss = self.loss_dict['seg_static_loss']
        dynamic_loss = self.loss_dict['seg_dynamic_loss']

        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || static Loss: %.4f || Dynamic Loss: %.4f" %
                  (epoch, batch_id + 1, batch_len, total_loss.item(), static_loss.item(), dynamic_loss.item()))
        else:
            pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || static Loss: %.4f || Dynamic Loss: %.4f" %
                                 (epoch, batch_id + 1, batch_len, total_loss.item(), static_loss.item(), dynamic_loss.item()))


        writer.add_scalar('Static_loss', static_loss.item(), epoch*batch_len + batch_id)
        writer.add_scalar('Dynamic_loss', dynamic_loss.item(), epoch*batch_len + batch_id)