...
def batch_split(dense_feature, record_len):
    cum_sum_len = list(np.cumsum(torch_tensor_to_numpy(record_len)))

    dense_feature = torch.tensor_split(dense_feature, cum_sum_len[:-1])

    return dense_feature


def batch_merge(dense_feature, record_len):
    """Merge Tensor [B, Max_Cav, C, H, W] to [All, C, H, W]"""
    slices = [dense_feature[i, :record_len[i]] for i in range(len(record_len))]
