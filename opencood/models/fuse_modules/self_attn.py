...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query, key, value, sttf_mask=None):
        score = torch.bmm(query, key.transpose(1, 2)) / self.sqrt_dim
        if sttf_mask is not None:
            score = score.masked_fill(sttf_mask == 0, -float('inf'))
...
class AttFusion(nn.Module):
    ...
    def forward(self, x, record_len):
        if record_len is None:
            L, C, W, H = x.shape
            xx = x.reshape(L, C, -1).permute(2, 0, 1)
            h = self.att(xx, xx, xx)
            h = h.permute(1, 2, 0).reshape(L, C, W, H)[0, ...]
            return h
        else:
