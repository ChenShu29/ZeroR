...
class V2XTEncoder(nn.Module):
    ...
    def forward(self, x, sttf_mask):
        ...
        # (B,L,H,W,3)
        prior_encoding = torch.zeros_like(x)[..., -3:]
        prior_encoding[..., -3] = torch.tensor(35.0 / 30, device=x.device)
        ...
        for attn, ff in self.layers:
            x = attn(x, mask=sttf_mask, prior_encoding=prior_encoding)
            x = ff(x) + x
        return x
...
class V2XTransformer(nn.Module):
    ...
    def forward(self, x, sttf_mask):
        output = self.encoder(x, sttf_mask)
        output = output[:, 0]
        return output
