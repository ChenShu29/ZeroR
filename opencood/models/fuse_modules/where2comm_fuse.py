    ...
    def forward(self, batch_confidence_maps, B):
        ...
        for b in range(B):
            ...
            if self.training:
                ...
                K = int(H * W * random.uniform(0.05, 0.55))
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, x, sttf_mask=None):
        cav_num, C, H, W = x.shape
        x = x.view(cav_num, C, -1).permute(2, 0, 1)  # (H*W, cav_num, C), perform self attention on each pixel
        if sttf_mask is not None:
            sttf_mask = sttf_mask.view(cav_num, 1, -1).permute(2, 0, 1)
        x = self.att(x, x, x, sttf_mask)
...
class Where2comm(nn.Module):
    ...
    def forward(self, x, psm_single, record_len, sttf_mask, backbone=None):
        """
        Fusion forwarding.
        ...
        if self.multi_scale:
            ...
            if self.fully:
                ...
                # Prune
                # batch_confidence_maps = self.regroup(psm_single, record_len)
                communication_masks, communication_rates = self.naive_communication(psm_single.unsqueeze(0), 1)
                x = x * communication_masks
            ...
            # x_fuse = torch.stack(x_fuse)
            x_fuse = self.fuse_modules(x, sttf_mask)
        return x_fuse, communication_masks
