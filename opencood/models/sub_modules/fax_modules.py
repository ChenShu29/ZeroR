...
def get_view_matrix(h, w, h_meters, w_meters, offset):
    """
...
class BEVEmbedding(nn.Module):
    def __init__(
        ...
        self.learned_features = nn.Parameter(
            sigma * torch.randn(dim, bev_height//upsample_scales[0], bev_width//upsample_scales[0]))  # d h w
...
class CrossViewSwapAttention(nn.Module):
    ...
    def forward(
        ...
        val = rearrange(val, 'b n d (x w1) (y w2) -> b n x y w1 w2 d',
                          w1=self.feat_win_size[0], w2=self.feat_win_size[1])  # window partition
        query = rearrange(self.cross_win_attend_1(query, key, val, skip=rearrange(x, 'b d (x w1) (y w2) -> b x y w1 w2 d', w1=self.q_win_size[0], w2=self.q_win_size[1]) if self.skip else None),'b x y w1 w2 d  -> b (x w1) (y w2) d')    # reverse window to feature
...
class FAXModule(nn.Module):
    def __init__(self, config, backbone_output_shape):
        super().__init__()
        ...
        for i, (feat_shape, num_layers) in enumerate(zip(self.backbone_output_shape, middle)):
            _, _, feat_dim, feat_height, feat_width = torch.zeros(feat_shape).shape
            ...
            if i < len(middle) - 1:
                downsample_layers.append(nn.Sequential(
                    nn.Sequential(
                        nn.Conv2d(dim[i], dim[i] // 4,  kernel_size=3, stride=1, padding=1, bias=False),
                        nn.PixelUnshuffle(2),
                        nn.Conv2d(dim[i+1], dim[i+1], kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, features, batch):
        b, v, _, _, _ = batch['inputs'].shape

        I_inv = batch['intrinsic'].inverse()
        # The 'extrinsic' here is the extrinsic parameter from camera coordinate to ego coordinate
        E_inv = batch['extrinsic']

        x = self.bev_embedding.get_prior()              # d H W
        x = repeat(x, '... -> b ...', b=b)  # b*l d H W

        for i, (cross_view, feature, layer) in enumerate(zip(self.cross_views, features, self.layers)):

            x = cross_view(i, x, self.bev_embedding, feature, I_inv, E_inv).contiguous()
            x = layer(x)
            if i < len(features)-1:
                down_sample_block = self.downsample_layers[i]
                x = down_sample_block(x)

                # # TODO: Temporal for umc
                # if i == len(features)-3:
                #     feature_v1 = x.clone()
