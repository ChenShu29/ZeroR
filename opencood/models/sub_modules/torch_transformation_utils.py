...
def get_roi_and_cav_mask(shape, spatial_correction_matrix,
    ...
    # (L,1,H,W)
    roi_mask = get_rotated_roi((L, C, H, W), T)
    # (H,W,1,L)
    com_mask = roi_mask.permute(2, 3, 1, 0)
    return com_mask
...
def get_rotated_roi(shape, correction_matrix):
    ...
    # (L,1,H,W)
    x = torch.ones((L, 1, H, W)).to(correction_matrix.dtype).to(correction_matrix.device)
    x = rearrange(x, 'l c h w  -> l c w h')
    x = torch.flip(x, dims=(3,))
    # (L,1,H,W)
    roi_mask = warp_affine(x, correction_matrix, dsize=(H, W), mode="nearest")
    roi_mask = torch.flip(roi_mask, dims=(3,))
    roi_mask = rearrange(roi_mask, 'l c w h  -> l c h w')
    # (L,C,H,W)
    roi_mask = torch.repeat_interleave(roi_mask, C, dim=1)
    return roi_mask
...
def _torch_inverse_cast(input):
    ...
    det = torch.det(input)[0]
    if det.abs() < 1e-8:
        epsilon = 1e-6
        input = input + epsilon * torch.eye(input.size(-1), device=input.device, dtype=input.dtype)
