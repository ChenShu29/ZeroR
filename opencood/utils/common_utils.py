...
def model_resave(model_pth, save_path):
    original_weights = torch.load(model_pth, map_location='cpu')
    target_layers = {
        "semantic_decoder.",
        "sematic_head.",
        "dynamic_head.",
    ...
    new_state_dict = {k: v for k, v in original_weights.items() if
                      any(k.startswith(prefix) for prefix in target_layers)}
    torch.save(new_state_dict, save_path)

    print(f"提取完成！共保存{len(new_state_dict)}个参数")


def compare_models(model1_path, model2_path, subnet_prefix):
    """
    比较两个模型中指定子网络的所有权重参数

    参数：
    model1_path (str): 第一个模型的路径
    model2_path (str): 第二个模型的路径
    subnet_prefix (str): 要比较的子网络前缀（如："backbone."）

    返回：
    bool: 所有参数相同返回True，否则返回False
    """
    # 加载模型到CPU
    ...
    # 提取state_dict
    state_dict1 = model1.state_dict() if isinstance(model1, torch.nn.Module) else model1
    state_dict2 = model2.state_dict() if isinstance(model2, torch.nn.Module) else model2

    # 提取子网络参数
    subnet1 = {k: v for k, v in state_dict1.items() if k.startswith(subnet_prefix)}
    subnet2 = {k: v for k, v in state_dict2.items() if k.startswith(subnet_prefix)}

    # 检查是否存在参数
    if len(subnet1) == 0:
        print(f"模型1中没有找到以'{subnet_prefix}'开头的参数")
        return False
    if len(subnet2) == 0:
        print(f"模型2中没有找到以'{subnet_prefix}'开头的参数")
        return False

    # 检查参数数量
    if len(subnet1) != len(subnet2):
        print(f"参数数量不同：模型1有{len(subnet1)}个，模型2有{len(subnet2)}个")
        return False

    # 检查所有参数键一致
    mismatch_keys = set(subnet1.keys()) ^ set(subnet2.keys())
    if mismatch_keys:
        print(f"键不匹配：{mismatch_keys}")
        return False

    # 逐个比较参数值
    for key in subnet1:
        if not torch.equal(subnet1[key], subnet2[key]):
            print(f"参数值不同：'{key}'")
            print(subnet1[key], subnet2[key])
            return False
