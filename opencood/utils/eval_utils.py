...
def calculate_list_average(tensor_list):
    sum_total = 0.0
    count_total = 0
    ...
    for sublist in tensor_list:
        for tensor in sublist:
            ...
            sum_total += detached_tensor.sum().item()
            count_total += detached_tensor.numel()
...
def eval_final_results_seg(iou_all, mean_iou_all, comm_rates=None, comm_volume=None, top_iou=30):
    class_ave_iou = np.mean(np.stack(iou_all, axis=0), axis=0) * 100
    mean_ave_iou = statistics.mean(mean_iou_all) * 100

    mean_iou_all = np.array(mean_iou_all)
    top5_indices = np.argsort(mean_iou_all)[-top_iou:][::-1]
    top5_indices = top5_indices.tolist()

    print('Class IoU: \nUnlabeled:%.2f;  Building:%.2f;  Fence:%.2f;  Pole:%.2f;  Road:%.2f;  SideWalk:%.2f;  Vegetation:%.2f;  Terrain:%.2f;  Lane:%.2f;  Vehicle:%.2f' % tuple(class_ave_iou))
    print('Mean IoU: %.2f' % mean_ave_iou)

    if comm_rates is not None:
        comm_rates = calculate_list_average(comm_rates) * 100
        print('Average Communication Rate is: %.2f' % comm_rates)
    if comm_volume is not None:
        comm_volume = calculate_list_average(comm_volume)

def eval_final_results_seg_multi(iou_all, mean_iou_all):
    # [PS, 10]
    class_ave_iou = np.mean(np.stack(iou_all, axis=0), axis=0) * 100
    # [PS,]
    mean_ave_iou = np.mean(np.stack(mean_iou_all, axis=0), axis=0) * 100

    for i in range(class_ave_iou.shape[0]):
        print("Predicted Timestamp: %d" % (i+1))
        print('Class IoU -- Unlabeled:%.2f;  Building:%.2f;  Fence:%.2f;  Pole:%.2f;  Road:%.2f;  SideWalk:%.2f;  Vegetation:%.2f;  Terrain:%.2f;  Lane:%.2f;  Vehicle:%.2f' % tuple(class_ave_iou[i]))
        print('Mean IoU: %.2f \n' % mean_ave_iou[i].item())